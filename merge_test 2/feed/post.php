  ...
  class Post {
    ...
    public static function getAllPostsPublic($conn){
      $posts = getRows($conn,
        ...
        where p.user = pf.ID and p.mode = 1","",array());
        ...
        for ($i = 0; $i < count($posts); $i++) {
          if ($posts[$i]['share_from'] != null) {
              ...
              FROM posts p, profiles u WHERE p.user = u.ID AND p.ID = ? p.mode=1";
    ...
    public static function getAllPostsFollowing($conn,$user_id){
      $posts = getRows($conn,
        "SELECT p.*, pf.display_name, pf.profile_image
        FROM posts p, relationships r, profiles pf
        WHERE (p.user = pf.ID) AND ((p.user = r.user2 AND r.user1 = ?) OR (p.user = ?))
        AND (p.mode = 2 OR p.mode = 3)
        ORDER BY p.date_created DESC","ii",array($user_id,$user_id));

        for ($i = 0; $i < count($posts); $i++) {
          if ($posts[$i]['share_from'] != null) {
              $query = "SELECT p.*, pf.display_name, pf.profile_image
              FROM posts p, relationships r, profiles pf
              WHERE (p.user = pf.ID) 
              AND ((p.user = r.user2 AND r.user1 = ? AND (p.share_from IS NULL
              OR EXISTS (SELECT * FROM relationships r1 WHERE r1.user2 = (SELECT user FROM posts WHERE ID = p.share_from) AND r1.user1 = ?)) OR (p.user = ?)))
              AND (p.mode = 2 OR p.mode = 3) 
